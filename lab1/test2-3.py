                    ...
                    if arp_src_mac in self.arp_in_port[dpid].keys():
                        ...
                        if req_dst_ip in self.arp_in_port[dpid][arp_src_mac].keys():
                            ...
                            if in_port != self.arp_in_port[dpid][arp_src_mac][req_dst_ip]:
                                ...
                                # drop the packet
                                match = parser.OFPMatch(in_port=in_port, arp_op=arp_pkt.opcode,
                                ...
                                # instruction = [parser.OFPInstructionActions(ofp.OFPIT_CLEAR_ACTIONS, [])]
                                # mod = parser.OFPFlowMod(datapath=dp, priority=20, command=ofp.OFPFC_ADD,
                                #                         idle_timeout=300, hard_timeout=600,
