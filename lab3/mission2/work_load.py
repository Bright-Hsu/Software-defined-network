...
class Workload(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.port_stat={}# (dpid,port_no):a list of port_stats
            ...
            for dpid in self.workload.keys():
                for port_no in self.workload[dpid].keys():
                    if self.workload[dpid][port_no] > 1:
                        print('Switch id dpid{} : <prot:{}  workload:{:.4f} Mbps >'
    ...
    def port_stats_reply_handler(self, ev):
        ...
        dpid = ev.msg.datapath.id
        self.workload.setdefault(dpid, {})
        self.port_stat.setdefault(dpid, {})
        for stat in sorted(body, key=attrgetter('port_no')):
            port_no = stat.port_no
...
############################get shortest(hop) path############################
        ...
        try:
            ...
            for i in range(0, len(paths)):
                ...
                for j in range(1, len(paths[i])-2):
                    cur_switch = paths[i][j]
                    next_switch = paths[i][j+1]
                    src = self.link_info[(cur_switch, next_switch)]
                    dst = self.link_info[(next_switch, cur_switch)]
                    temp = 1000 - max(self.workload[cur_switch][src], self.workload[next_switch][dst])
                    neck.append(temp)
                neck_path.append(min(neck))

            print("neck_path:")
            print(neck_path)
            index = neck_path.index(max(neck_path))
            print(paths[index])
            return paths[index]
        except:
    ...
    def send_flow_mod(self, parser, dpid, pkt_type, src_ip, dst_ip, in_port, out_port):
        ...
        actions = [parser.OFPActionOutput(out_port)]
        self.add_flow(dp, 5, match, actions, 150, 300)
